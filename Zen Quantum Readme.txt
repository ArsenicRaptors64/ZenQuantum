		...
		THIS PRODUCT IS FOR INTERNAL USE ONLY
...
latest driver and pasting it to the installation directory and 
navigating to:

<INSERT A DIRECTORY URI HERE>

Once navigated to the directory, rename the "SQLite_Library.jar" 
in the folder by appending ".bak" to the file (incase the latest 
release of the SQLite Library is unstable) and paste the newly 
downloaded library in the above directory, and rename it to 
"SQLite_Library.jar" and the JDBC driver is updated to latest
version!

The automated setup will ask for a few details and voila! The program 
is ready for use.

II) CLIENT PROGRAM

The program can be downloaded officially from GitHub. The program can 
be unzipped anywhere and can be run from a terminal in two ways:

i)  Running the Run_Program.bat or Run_Program.sh in a console/terminal
ii) using the command "java Launcher.java"

Once done, the program will ask the link to the server program, which is
provided by a host. Once the link has been established, the program will
ask the user to login. If the user is not logged in, they can sign up
and create a standard user. The person can then login and specify the
name of the session and then join it after providing the session password.
(The session password is given by the host)
The user can then join the session and participate in the session.
...
This program is a guide for both the Server and Client programs.
